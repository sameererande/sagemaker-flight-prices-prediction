...
def remove_missing_info(data, threshold_pct=0):
    # Identify columns with missing values
    na_cols = [col for col in data.columns if data[col].isna().any()]
    
    # Remove rows with missing values (optional: you can drop columns too)
    # Drop rows where any column has missing value
    cleaned_data = data.dropna()
    
    # Alternatively, drop columns with missing values exceeding the threshold
    if threshold_pct > 0:
        # Calculate the percentage of missing values for each column
        na_pct = data.isna().mean() * 100
        cols_to_drop = na_pct[na_pct > threshold_pct].index
        
        # Drop the columns that exceed the threshold percentage
